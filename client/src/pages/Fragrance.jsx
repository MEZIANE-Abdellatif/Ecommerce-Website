...
const Fragrance = () => {
  ...
  const fetchFragranceProducts = useCallback(async () => {
    try {
      ...
      if (selectedFragranceFamily && selectedFragranceFamily !== 'all') {
        params.append('fragranceFamily', selectedFragranceFamily);
      }
      if (selectedConcentration && selectedConcentration !== 'all') {
  ...
  const fragranceFamilies = [
    ...
    { id: 'floral', label: 'Floral', icon: '🌸', color: 'from-purple-400 to-rose-500', description: 'Delicate blooms' },
    { id: 'woody', label: 'Woody', icon: '🌳', color: 'from-rose-400 to-pink-500', description: 'Rich & warm' },
    { id: 'citrus', label: 'Citrus', icon: '🍊', color: 'from-pink-400 to-purple-500', description: 'Fresh & vibrant' },
    { id: 'oriental', label: 'Oriental', icon: '🌙', color: 'from-purple-400 to-rose-500', description: 'Exotic & sensual' },
    { id: 'fresh', label: 'Fresh', icon: '💧', color: 'from-rose-400 to-pink-500', description: 'Clean & crisp' },
    { id: 'gourmand', label: 'Gourmand', icon: '🍰', color: 'from-pink-400 to-purple-500', description: 'Sweet & edible' },
    { id: 'aquatic', label: 'Aquatic', icon: '🌊', color: 'from-purple-400 to-rose-500', description: 'Ocean breeze' }
  ];

  const concentrations = [
    { id: 'all', label: 'All Concentrations', icon: '✨' },
    { id: 'EDT', label: 'EDT', icon: '💧', description: 'Eau de Toilette' },
    { id: 'EDP', label: 'EDP', icon: '💎', description: 'Eau de Parfum' },
    { id: 'parfum', label: 'Parfum', icon: '👑', description: 'Pure Perfume' },
    { id: 'cologne', label: 'Cologne', icon: '🧴', description: 'Light & Fresh' }
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-rose-50 pt-0 relative overflow-hidden">
      {/* Animated Scent Particles with Mazzinka Colors */}
      <div className="absolute inset-0 overflow-hidden">
        {/* Floating Perfume Bottles */}
        <div className="absolute top-10 left-10 w-32 h-32 bg-gradient-to-br from-pink-300/20 to-purple-300/20 rounded-full animate-ping opacity-60 transform rotate-12"></div>
        <div className="absolute top-20 right-20 w-24 h-24 bg-gradient-to-br from-purple-300/20 to-rose-300/20 rounded-full animate-ping opacity-60 transform -rotate-12" style={{ animationDelay: '1s' }}></div>
        <div className="absolute bottom-20 left-1/3 w-20 h-20 bg-gradient-to-br from-rose-300/20 to-pink-300/20 rounded-full animate-ping opacity-60 transform rotate-45" style={{ animationDelay: '2s' }}></div>
        
        {/* Scent Molecules */}
        <div className="absolute top-1/4 left-1/4 w-2 h-2 bg-pink-300 rounded-full animate-ping opacity-60"></div>
        <div className="absolute top-1/3 right-1/3 w-1 h-1 bg-purple-300 rounded-full animate-ping opacity-60" style={{ animationDelay: '0.5s' }}></div>
        <div className="absolute bottom-1/3 left-1/4 w-3 h-3 bg-rose-300 rounded-full animate-ping opacity-60" style={{ animationDelay: '1.5s' }}></div>
        <div className="absolute top-1/2 right-1/4 w-2 h-2 bg-pink-300 rounded-full animate-ping opacity-60" style={{ animationDelay: '0.8s' }}></div>
      </div>

      {/* Hero Section with Scent-Inspired Design and Mazzinka Identity */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          <nav className="text-sm text-gray-600 mb-8">
            <div className="inline-flex items-center space-x-3 bg-white/70 backdrop-blur-xl rounded-full px-8 py-4 shadow-2xl border border-white/30">
              ...
              <span className="text-gray-900 font-bold">Fragrance</span>
            </div>
          </nav>
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Discover our collection of premium fragrance products
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20 relative z-10">
        {/* Simple Fragrance Filter System */}
        <div className="mb-16">
          <div className="bg-white/70 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-white/30">
            <h3 className="text-lg sm:text-xl font-bold text-gray-800 mb-6 sm:mb-8 text-center">Find Your Perfect Scent</h3>
            
            {/* Fragrance Filters */}
            <div className="space-y-6">
              {/* Fragrance Family Selection */}
              <div className="space-y-4">
                <label className="text-lg font-semibold text-gray-700 block">Choose Your Fragrance Family</label>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {fragranceFamilies.map((family, index) => (
                    ...
                    >
                      <div className="font-bold text-sm mb-1">{family.label}</div>
                      <div className="text-xs opacity-80">{family.description}</div>
                      
                      {/* Active Indicator */}
                      {selectedFragranceFamily === family.id && (
                        <div className="absolute -bottom-1 left-1/2 transform -translate-x-1/2 w-2 h-2 bg-white rounded-full animate-ping shadow-lg"></div>
                      )}
                    </button>
                  ))}
                </div>
              ...
              {/* Concentration & Sort Selection */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Concentration */}
                <div className="space-y-4">
                  <label className="text-lg font-semibold text-gray-700 block">Concentration</label>
                  <select 
                  ...
                  >
                    {concentrations.map((concentration) => (
                      <option key={concentration.id} value={concentration.id}>
                        {concentration.label} - {concentration.description}
                      </option>
                    ))}
                  </select>
                </div>
