...
const Makeup = () => {
  ...
  const [selectedSkinPart, setSelectedSkinPart] = useState('all');
  const [sortBy, setSortBy] = useState('featured');

  const fetchMakeupProducts = useCallback(async () => {
    try {
      const params = new URLSearchParams();
      params.append('category', 'makeup');
      if (selectedSkinPart && selectedSkinPart !== 'all') {
  ...
  useEffect(() => {
    fetchMakeupProducts();
  }, [selectedSkinPart, fetchMakeupProducts]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-rose-50 pt-0">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative z-10">
          ...
          <nav className="text-sm text-gray-600 mb-8">
            <div className="inline-flex items-center space-x-3 bg-white/60 backdrop-blur-md rounded-full px-8 py-4 shadow-lg">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Discover our collection of premium makeup products
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        {/* Unified Makeup Filter System */}
        <div className="mb-16">
          <div className="bg-white/70 backdrop-blur-xl rounded-3xl p-4 sm:p-6 lg:p-8 shadow-2xl border border-white/30">
            <h3 className="text-lg sm:text-xl font-bold text-gray-800 mb-6 sm:mb-8 text-center">Find Your Perfect Makeup</h3>
            
            {/* Mobile-First Filter Layout */}
            <div className="space-y-6">
              {/* Sort Options - Top on mobile, better positioning */}
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <label className="text-lg sm:text-xl font-semibold text-gray-700 text-center sm:text-left">Choose Your Makeup Area</label>
                <div className="flex items-center justify-center sm:justify-end space-x-2">
                  <label className="text-sm font-semibold text-gray-700">Sort:</label>
                  <select
                    value={sortBy}
                    onChange={(e) => setSortBy(e.target.value)}
                    className="p-2 sm:p-3 bg-white/80 border-2 border-transparent hover:border-purple-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-300 shadow-md text-xs sm:text-sm min-w-[120px] sm:min-w-[140px]"
                  >
              ...
              {/* Filter Buttons - Mobile-first grid layout */}
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
                {[
                  ...
                  <button
                    key={part.id}
                    onClick={() => setSelectedSkinPart(part.id)}
                    className={`p-3 sm:p-4 rounded-xl text-center transition-all duration-300 transform hover:scale-105 ${
                      selectedSkinPart === part.id
                  ...
                  >
                    <div className="text-2xl sm:text-3xl mb-1 sm:mb-2">{part.icon}</div>
                    <div className="text-xs sm:text-sm font-medium">{part.label}</div>
                  </button>
                ))}
              </div>
            </div>
      ...
      <div className="fixed top-1/4 left-10 w-4 h-4 bg-pink-400 rounded-full opacity-60 animate-pulse" style={{ animationDelay: '0s' }}></div>
      <div className="fixed top-1/3 right-20 w-3 h-3 bg-purple-400 rounded-full opacity-60 animate-pulse" style={{ animationDelay: '1s' }}></div>
      <div className="fixed bottom-1/4 left-1/4 w-2 h-2 bg-rose-400 rounded-full opacity-60 animate-pulse" style={{ animationDelay: '2s' }}></div>
    </div>
  );
};
