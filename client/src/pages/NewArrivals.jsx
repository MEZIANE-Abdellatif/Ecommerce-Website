...
const NewArrivals = () => {
  ...
  const fetchNewArrivalsProducts = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/products?sort=newest');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-rose-50 pt-0 relative overflow-hidden">
      {/* Animated Arrival Background with Mazzinka Colors */}
      <div className="absolute inset-0 overflow-hidden">
        ...
        {/* Trails */}
        <div className="absolute top-1/4 left-1/4 w-1 h-16 bg-gradient-to-b from-pink-300/40 to-transparent animate-pulse opacity-60"></div>
        <div className="absolute top-1/3 right-1/3 w-1 h-20 bg-gradient-to-b from-purple-300/40 to-transparent animate-pulse opacity-60" style={{ animationDelay: '0.5s' }}></div>
        <div className="absolute bottom-1/3 left-1/4 w-1 h-12 bg-gradient-to-b from-rose-300/40 to-transparent animate-pulse opacity-60" style={{ animationDelay: '1.5s' }}></div>
      </div>

      {/* Hero Section with Arrival-Inspired Design and Mazzinka Identity */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20 relative z-10">
        {/* Interactive Sort Options with Mazzinka Identity */}
        <div className="mb-16">
          <div className="bg-white/70 backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-white/30">
            <h3 className="text-lg sm:text-xl font-bold text-gray-800 mb-6 sm:mb-8 text-center">Sort Your Discovery Journey</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
              {sortOptions.map((option, index) => (
                <button
                  key={option.value}
                  onClick={() => setSortBy(option.value)}
                  className={`group relative p-6 rounded-2xl text-center transition-all duration-700 transform hover:scale-110 hover:-translate-y-3 ${
                    sortBy === option.value
                      ? 'bg-gradient-to-r from-pink-500 to-rose-500 text-white shadow-2xl'
                      : 'bg-white/80 text-gray-700 border-2 border-transparent hover:border-pink-300 hover:bg-white shadow-xl'
                  }`}
                ...
                >
                  ...
                  {/* Active Indicator with Mazzinka Colors */}
                  {sortBy === option.value && (
                    <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 w-4 h-4 bg-white rounded-full animate-ping shadow-lg"></div>
                  )}
                  
                  {/* Hover Glow Effect with Mazzinka Colors */}
                  <div className={`absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${
                    sortBy === option.value ? 'bg-white/20' : 'bg-gradient-to-r from-pink-400/20 to-rose-400/20'
                  }`}></div>
                </button>
              ))}
            </div>
            ...
            >
              ...
              <div className="relative bg-white/80 backdrop-blur-xl rounded-3xl p-6 shadow-2xl border border-white/30 hover:shadow-pink-500/25 transition-all duration-500 transform group-hover:-translate-y-2 group-hover:scale-102 h-full flex flex-col">
                ...
                <div className="relative overflow-hidden rounded-2xl mb-6 group-hover:rounded-3xl transition-all duration-500 flex-shrink-0">
                  ...
                  {/* NEW Badge with Rotation Effect */}
                ...
                <div className="flex flex-col h-full">
                  ...
                  <div className="mt-auto">
                    ...
                    >
                      ...
                      <div className="relative bg-gradient-to-r from-pink-500 to-rose-500 text-white py-4 px-6 rounded-2xl font-bold text-lg shadow-xl transform group-hover/btn:scale-105 transition-all duration-500">
                        Discover
